        ...
        const templateData = [
            {
                ...
                '承运人联系方式': '13800138000',
                '承运人车牌': '粤B12345',
                '承运类型（1：司机直送/2：承运商）': 1,  // 1-司机直送，2-承运商
                '运费': '2000'
            },
            {
                '子订单号': 'JD202401010003-1',
                '承运人名称': '李四物流有限公司',
                '承运人联系方式': '13900139000',
                '承运人车牌': '粤A67890',
                '承运类型（1：司机直送/2：承运商）': 2,  // 1-司机直送，2-承运商
                '运费': '3000'
            }
        ];
        ...
        reader.onload = async (e) => {
            try {
                ...
                const deliveries = jsonData.map((row, index) => {
                    ...
                    if (!row['承运类型（1：司机直送/2：承运商）']) errors.push(`第${rowNum}行：承运类型不能为空`);

                    // 验证承运类型
                    const carrierType = parseInt(row['承运类型（1：司机直送/2：承运商）']);
